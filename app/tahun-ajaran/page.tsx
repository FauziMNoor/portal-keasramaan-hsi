...
export default function TahunAjaranPage() {
  ...
  const resetForm = () => {
    setFormData({ tahun_ajaran: '', status: 'aktif' });
    setEditMode(false);
    setCurrentId(null);
  };
      ...
      <main className="flex-1 p-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-gray-800 mb-2">Tahun Ajaran</h1>
              <p className="text-gray-600">Kelola data tahun ajaran keasramaan</p>
            </div>
            <button
              onClick={() => {
                resetForm();
                  ...
                  <tbody>
                    {data.map((item, index) => (
                      <tr key={item.id} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                        ...
                        <td className="px-6 py-4 text-gray-800 font-medium">{item.tahun_ajaran}</td>
