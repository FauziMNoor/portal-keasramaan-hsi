...
export default function AsramaPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    const { data: result, error } = await supabase.from('asrama_keasramaan').select('*').order('created_at', { ascending: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editMode && currentId) {
      await supabase.from('asrama_keasramaan').update({ ...formData, updated_at: new Date().toISOString() }).eq('id', currentId);
    } else {
      await supabase.from('asrama_keasramaan').insert([formData]);
    }
    setShowModal(false);
    resetForm();
    fetchData();
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Yakin ingin menghapus data ini?')) {
      await supabase.from('asrama_keasramaan').delete().eq('id', id);
      fetchData();
    }
  };
      ...
      <main className="flex-1 p-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-gray-800 mb-2">Asrama</h1>
              <p className="text-gray-600">Kelola data asrama keasramaan</p>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-gray-800">{editMode ? 'Edit Asrama' : 'Tambah Asrama'}</h2>
              </div>
